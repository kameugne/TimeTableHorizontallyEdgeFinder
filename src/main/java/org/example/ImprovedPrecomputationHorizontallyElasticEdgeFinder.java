...
public class ImprovedPrecomputationHorizontallyElasticEdgeFinder {
    Integer C;
    Task[] tasks;
    Profile tl;
    Timepoint tinit;
    ...
    public ImprovedPrecomputationHorizontallyElasticEdgeFinder(Task[] tasks, int C)
    {
        this.C = C;
        this.tasks = tasks;
        this.tl = new Profile();
    ...
    private boolean EdgeFinder_Detection()
    {
        ...
        if(makespan > tasks[tasks_indices_lct[tasks_indices_lct.length - 1]].latestCompletionTime())
            return false;
        int[] fpEnergy = energyExternalTasks();
            ...
            if (tasks[tasks_indices_lct[i]].earliestCompletionTime() < tasks[tasks_indices_lct[i]].latestCompletionTime()) {
                ...
                if (index != -1) {
                    ...
                    if (ect > Lct) {
                        ...
                        int En = 0; int j1 = -1; int Exp1 = Integer.MAX_VALUE;
                        int j2 = -1; int Exp2 = Integer.MAX_VALUE; boolean isDetected = false;
                        for (int j = 0; j < tasks.length; j++) {
                            if (tasks[tasks_indices_lct[j]].latestCompletionTime() < tasks[tasks_indices_lct[i]].latestCompletionTime()) {
                                ...
                                if (tasks[tasks_indices_lct[j]].latestCompletionTime() <= tasks[tasks_indices_lct[i]].earliestCompletionTime() &&
                                    ...
                                    int exp = (C - tasks[tasks_indices_lct[i]].height()) * tasks[tasks_indices_lct[j]].latestCompletionTime() - En - fpEnergy[j] + energyExternalTasks(j,i);
                                ...
                                if (tasks[tasks_indices_lct[j]].latestCompletionTime() > tasks[tasks_indices_lct[i]].earliestCompletionTime()) {
                                    int exp = C * tasks[tasks_indices_lct[j]].latestCompletionTime() - En - fpEnergy[j] + energyExternalTasks(j,i);
    ...
    private int[] energyExternalTasks() {
        int[] en = new int[tasks.length];
        for (int j = 0; j < tasks.length; j++) {
            int E = 0;
            for (int i = j+1; i < tasks.length; i++) {
                if (tasks[tasks_indices_lct[i]].latestCompletionTime() > tasks[tasks_indices_lct[j]].latestCompletionTime() &&
                   tasks[tasks_indices_lct[i]].hasFixedPart() && tasks[tasks_indices_lct[i]].latestStartingTime() < tasks[tasks_indices_lct[j]].latestCompletionTime()) {
                    E += tasks[tasks_indices_lct[i]].height() * Math.max(0, Math.min(tasks[tasks_indices_lct[i]].earliestCompletionTime(), tasks[tasks_indices_lct[j]].latestCompletionTime()) - tasks[tasks_indices_lct[i]].latestStartingTime());
                }
            }
            en[j] = E;
    ...
    private int energyExternalTasks(int j, int i) {
        int E = 0;
        if (tasks[tasks_indices_lct[i]].latestCompletionTime() > tasks[tasks_indices_lct[j]].latestCompletionTime() &&
                tasks[tasks_indices_lct[i]].hasFixedPart() && tasks[tasks_indices_lct[i]].latestStartingTime() < tasks[tasks_indices_lct[j]].latestCompletionTime()) {
            E += tasks[tasks_indices_lct[i]].height() * Math.max(0, Math.min(tasks[tasks_indices_lct[i]].earliestCompletionTime(), tasks[tasks_indices_lct[j]].latestCompletionTime()) - tasks[tasks_indices_lct[i]].latestStartingTime());
    ...
    private int InitializeIncrementsEFSet(int u) {
        ...
        for(int i = 0; i < tasks.length; i++)
        {
            if (tasks[tasks_indices_lct[i]].latestCompletionTime() < tasks[tasks_indices_lct[u]].latestCompletionTime()) {
                ...
                if (i != u && index!= -1 &&  /*tasks[tasks_indices_lct[i]].latestCompletionTime() > Lct &&*/
                        tasks[tasks_indices_lct[i]].hasFixedPart() && tasks[tasks_indices_lct[i]].latestStartingTime() < Lct) {
                    t = tasks[tasks_indices_lct[i]].lst_to_timepoint;
                    t.increment += tasks[tasks_indices_lct[i]].height();
                    t.incrementMax += tasks[tasks_indices_lct[i]].height();
                    if (tasks[tasks_indices_lct[i]].earliestCompletionTime() < Lct) {
                        t = tasks[tasks_indices_lct[i]].ect_to_timepoint;
                        t.increment -= tasks[tasks_indices_lct[i]].height();
                        t.incrementMax -= tasks[tasks_indices_lct[i]].height();
                    } else {
    ...
    private void InitializeIncrementsDetect(int maxIndex, int u)
    {
        ...
        for(int i = 0; i < tasks.length; i++)
        {
            if (tasks[tasks_indices_lct[i]].latestCompletionTime() <= tasks[tasks_indices_lct[maxIndex]].latestCompletionTime()) {
                ...
                if (i != u && tasks[tasks_indices_lct[i]].hasFixedPart() &&
